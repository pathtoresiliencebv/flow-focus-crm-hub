-- Fix chat system: ensure proper foreign keys and clean up inconsistencies
-- This migration ensures the direct_messages table is properly set up with foreign keys

-- First, let's check if we have the correct table structure
DO $$
BEGIN
  -- Drop the older direct_messages table if it exists with the old schema (sender_id/receiver_id)
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'direct_messages' AND column_name = 'sender_id'
  ) THEN
    DROP TABLE IF EXISTS direct_messages CASCADE;
  END IF;
END $$;

-- Recreate direct_messages table with proper structure and foreign keys
...
-- Add indexes for performance (drop if exist first)
...
-- Trigger for updated_at (ensure the function exists)
...
-- Make sure the get_available_chat_users function is up to date
